...
#include<mask.hpp>
    ...
    void Server::addRootSelectInput(long _eventMask){
        ...
        else eventMask.add(_eventMask);
    }

    void Server::grabAnyKey(bool condition){
        if (condition){
            XGrabKey(display, AnyKey, AnyModifier, DefaultRootWindow(display), false, GrabModeAsync, GrabModeSync);
            XFlush(display);
        }
    }
    ...
    void Server::grabAnyButton(bool condition){
        if (condition){
            XGrabButton(display, AnyButton, AnyModifier, DefaultRootWindow(display), false, ButtonReleaseMask|ButtonPressMask|PointerMotionMask ,GrabModeSync, GrabModeAsync, None,None);
            XFlush(display);
        }
    }
    ...
    void Server::listen(){
        grabAnyKey( eventMask.contains(KeyPressMask) || eventMask.contains(KeyReleaseMask));
        grabAnyButton( eventMask.contains(ButtonPressMask) || eventMask.contains(ButtonReleaseMask) || eventMask.contains(PointerMotionMask));
        ...
        for(;;){
            ...
            XAllowEvents(display,ReplayKeyboard, getEvent()->xkey.time);
        }
    }
